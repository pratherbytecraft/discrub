...
describe('Appearance menu (2.2.0)', () => {
  ...
  it('names the current layout and theme, opens on click, and lists the six layouts', () => {
    ...
    // Only Classic is built so far: the rest read as coming soon and cannot be picked.
    cy.get('[data-testid="layout-card-native"]').should('be.disabled');
  ...
  it('shows the layout block in Settings > Display with Classic selected', () => {
    ...
    cy.get('[data-testid="settings-layout-native"]').should('be.disabled');
